...
-- Create improved search insights function without ORDER BY in DISTINCT aggregation
...
BEGIN
  ...
  SELECT 
    ...
    (
      SELECT jsonb_agg(x ORDER BY (x->>'timestamp')::timestamptz DESC)
      FROM jsonb_array_elements(s.search_details) x
      LIMIT 10
    ) as top_results
